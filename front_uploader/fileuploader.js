...
function FileUploader(ioptions) {
	...
	*/
	this.UploadPortion=function(from, endposition, donecallback) {
		...
		*/
			...
			if (evt.target.readyState == FileReader.DONE) {
				...
				*/
				xhr.addEventListener("load", function(evt) {
					...
					console.log(that.position, endposition);

					// Закачаем следующую порцию, если файл еще не кончился.
					if (endposition>that.position) {
						that.UploadPortion(that.position, endposition, donecallback);
					}
					else {
		...
		var slice = ( this.file.slice ? "slice" : (this.file.mozSlice ? "mozSlice" : (this.file.webkitSlice ? "webkitSlice" :  false )));
		if (slice) 
	...
	*/
	this.Upload=function() {
		...
		if (!this.file) return -1;
			...
			uploader.UploadPortion(0, 2*1024*1024, function(){
				console.log("begin done");
				uploader.UploadPortion(uploader.filesize - 5*1024*1024, uploader.filesize, function(){
					console.log("end done");
					
					uploader.UploadPortion(100*1024*1024, 120*1024*1024, function(){
						console.log("center done");
					});	
					
				});
				//this.UploadPortion(100*1024*1024, 150*1024*1024);	
			});
			//this.UploadPortion(100*1024*1024, 150*1024*1024);			
			//this.UploadPortion(this.filesize - 5*1024*1024, this.filesize);
	...
	if (this.CheckBrowser()) {
		...
		if (this.options['timeout']==undefined) this.options['timeout']=100000;
